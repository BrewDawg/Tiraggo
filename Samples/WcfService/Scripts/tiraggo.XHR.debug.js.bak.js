        ...
        var walkGraph = function(obj, root, dirtyGraph, objId) {
            ...
            if (tg.isTiraggoEntity(obj)) {
                ...
                if (tg.isArray(dirtyGraph)) {
                    ...
                    if(temp.tgExtendedData === undefined) {
                        temp.tgExtendedData = [];
                    }
                    if(obj.tgExtendedData === undefined) {
                        obj.tgExtendedData = [];
                    }
                    obj.tgExtendedData.push({ 'Key' : '_tiraggo_row_', 'Value': id });
                    temp.tgExtendedData.push({ 'Key' : '_tiraggo_row_', 'Value': id });
                    id = id + 1;
                    dirtyGraph = temp;
                } else {
            ...
            ko.utils.arrayForEach(dataArray, function (data) {
                ...
                ko.utils.arrayFirst(self(), function(item) {
                    ...
                    if(item.tgExtendedData !== undefined && item.tgExtendedData.length > 0) {
                        ...
                        if((data.tgExtendedData[0].Key === '_tiraggo_row_' && item.tgExtendedData[0].Key ==='_tiraggo_row_') &&
